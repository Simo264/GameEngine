...
vec3 CalculateBlinnPhongLight(PointLight light, vec3 normal, vec3 viewDir) 
{
  ...
  if(IsNormalMapActive() && HasNormalTexture())
  {
    ...
    lightDirection = normalize(tangentLightPosition - TangentFragPos);
  }
  else
  {
  ...
  // Diffuse shading
  const float diffFactor = max(dot(normal, lightDirection), 0.0);
...
vec3 CalculateSpotLight(SpotLight light, vec3 normal, vec3 viewDir)
{
  ...
  // Diffuse
  const float diffFactor = max(dot(normal, lightDirection), 0.0f);
  vec3 diffuse = (light.color * light.intensity) * diffFactor * g_diffuseColor.rgb;
