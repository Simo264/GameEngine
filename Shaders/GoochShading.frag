...
vec3 f_unlit(vec3 c_cool)
{
  return c_cool;
}

vec3 f_lit(vec3 l, vec3 n, vec3 v, vec3 c_cool, vec3 c_warm, vec3 c_highlight) 
{
  float t = (dot(n, l) + 1.0f) * 0.5f;  // Interpolazione tra cool e warm
  vec3 c_toon = mix(c_cool, c_warm, t);

  // Calcolo della specularità
  vec3 r = reflect(-l, n);
  float s = pow(max(dot(r, v), 0.0), shininess); // Controllo più morbido della specularità
  return (s * c_highlight + (1.0 - s) * c_toon);
}
