...
Texture2D* ObjectLoader::GetTexture(aiTextureType materialType) const
{
  if (material->GetTextureCount(materialType) <= 0)
    return nullptr;

  aiString fileName;
  if (material->GetTexture(materialType, 0, &fileName) != AI_SUCCESS)
    return nullptr;
