...
void Texture2D::CreateStorage(Texture2DInternalFormat internalFormat, i32 width, i32 height) const
{
  ...
  glTextureStorage2D(id, mipmapLevels, static_cast<u32>(internalFormat), width, height);
}
void Texture2D::CreateStorageMultisampled(Texture2DInternalFormat internalFormat, i32 samples, i32 width, i32 height) const
{
  glTextureStorage2DMultisample(id, samples, static_cast<u32>(internalFormat), width, height, true);
}
...
{
  glClearTexImage(id, level, static_cast<u32>(GetFormat()), static_cast<u32>(type), data);
}

void Texture2D::GetTextureImage(i32 level, Texture2DGetImageType type, i32 buffSize, void *pixels) const
{
  glGetTextureImage(id, level, static_cast<u32>(GetFormat()), static_cast<u32>(type), buffSize, pixels);
}
...
Texture2DFormat Texture2D::GetFormat() const
{
  ...
  default:
    CONSOLE_WARN("Unknown internal format: {:#x} - Defaulting to Texture2DFormat::RED", static_cast<u32>(internalFormat));
    return Texture2DFormat::RED;
  }
}
i32 Texture2D::GetNumChannels() const
{
  ...
  default:
    CONSOLE_WARN("Unknown internal format: {:#x} - Defaulting to 1 channel", static_cast<u32>(internalFormat));
    return 1;
  }
}
